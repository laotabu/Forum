<?php
	...
	$sql="select * from `post` where `audit_result`=1 and `inform`!=0 order by post_time desc";
	$data=json_decode($mysql->queryAll($sql));
	
	/* 模糊查询 */
	if(isset($_GET['select'])){
		$text=$_GET['select'];
		$sql="SELECT * FROM `post` where `post_title` Like '%$text%' and `audit_result`=1 and `inform`!=0 order by post_time desc";
		$data=json_decode($mysql->queryAll($sql));
	}
	/* 分页 */
...
<html>
	...
	<script>
		$(function(){
			...
			$("#nav .ul1 .post").text("用户举报");
			$("#nav .ul2 li:contains('用户举报')").css("background","#1d1d82");
